...
class IkLegRig(module.RigModule, mixin.JointMixin, mixin.ControlMixin):
    ...
    def create(self, *args, **kwargs):
        super(IkLegRig, self).create(*args, **kwargs)
    ...
    def _create_buffer_joint(self):
        ...
        buffer_joint = tp.Dcc.duplicate_object(ik_chain[-1].meta_node, only_parent=True)
        tp.Dcc.set_parent(ik_chain[-1].meta_node, buffer_joint)
        if not tp.Dcc.is_attribute_connected_to_attribute(
                buffer_joint, 'scale', ik_chain[-1].meta_node, 'inverseScale'):
            tp.Dcc.connect_attribute(buffer_joint, 'scale', ik_chain[-1].meta_node, 'inverseScale')

        for axis in 'XYZ':
            for attr_name in ['rotate', 'jointOrient']:
                tp.Dcc.set_attribute_value(ik_chain[-1].meta_node, '{}{}'.format(attr_name, axis), 0)

        return buffer_joint

    def _create_top_control(self, force_locator=False):
        """
        ...
        if self.top_control_as_locator or force_locator:
    ...
    def _create_bottom_control(self):
        ...
        if self.orient_constraint:
            if self.create_sub_controls:
                tp.Dcc.create_orient_constraint(ik_chain[-1].meta_node, sub_control.meta_node, maintain_offset=True)
            else:
                tp.Dcc.create_orient_constraint(ik_chain[-1].meta_node, bottom_control.meta_node, maintain_offset=True)
    ...
    def _get_pole_joints(self, as_meta=True):
        ...
        if not self.has_attr('pole_angle_joints') or not self.pole_angle_joints:
            ...
            pole_angle_joints = [ik_chain[0], mid_joint, ik_chain[-1]]
