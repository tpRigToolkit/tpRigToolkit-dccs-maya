...
class MuscleSpline(object):
    def __init__(self, **kwargs):
        ...
        self._num_controls = kwargs.pop('num_controls', 3)
        self._control_type = kwargs.pop('control_type', 'cube')
        self._num_drivens = kwargs.pop('num_drivens', 5)
        self._driven_type = kwargs.pop('driven_type', 'joint')
        self._constrain_mid = kwargs.pop('constrain_mid', False)
        self._lock_scale = kwargs.pop('lock_scale', True)
        self._lock_jiggle_attributes = kwargs.pop('lock_jiggle_attributes', False)
        self._ctrl_suffix = kwargs.pop('control_suffix', 'ctrl')
        self._jnt_suffix = kwargs.pop('joint_suffix', 'jnt')
        self._grp_suffix = kwargs.pop('group_suffix', 'grp')
        self._driven_suffix = kwargs.pop('driven_suffix', 'drv')
        self._main_set_name = kwargs.pop('main_set_name', 'setMUSCLERIGS')
        self._rig_set_suffix = kwargs.pop('rig_set_suffix', 'RIG')
        ...
        self._root_suffix = kwargs.pop('root_suffix', 'root')
        self._auto_suffix = kwargs.pop('auto_suffix', 'auto')
    ...
    def create(self):
        ...
        if not maya.cmds.objExists(self._main_set_name):
            maya.cmds.sets(name=self._main_set_name, empty=True)

        set_rig = 'set{}{}'.format(base_name, self._rig_set_suffix)
        if not maya.cmds.objExists(set_rig):
            maya.cmds.sets(name=set_rig, empty=True)
            maya.cmds.sets(set_rig, include=self._main_set_name)

        test1 = maya.cmds.objExists('{}_{}'.format(self._muscle_spline_name, base_name))
        test2 = maya.cmds.objExists('{}_{}'.format(self._muscle_spline_name, self._grp_suffix))
        if test1 or test2:
            msg = 'A muscle or spline with given name "{}" already exists.' \
        ...
        self._main_group = maya.cmds.group(
            name='{}_{}_{}'.format(base_name, self._muscle_spline_name, self._grp_suffix), empty=True, world=True)
        maya.cmds.sets(self._main_group, include=set_rig)
        ...
        maya.cmds.sets(self._spline_node, include=set_rig)
        ...
        maya.cmds.sets(self._controls_group, include=set_rig)

        # create drivens group
        self._drivens_group = maya.cmds.group(name='{}_{}_{}'.format(base_name, self._muscle_spline_name, self._joints_group_suffix), empty=True, world=True)
        ...
        maya.cmds.sets(self._drivens_group, include=set_rig)
        ...
        for i in range(self._num_controls):
            ctrl_name = '{}_{}_{}_{}'.format(base_name, self._muscle_spline_name, i, self._ctrl_suffix)
            new_ctrl = MuscleSplineControl(ctrl_name, self._control_type, self._size).create()
            root_grp = maya.cmds.group(
                name=ctrl_name.replace(self._ctrl_suffix, self._root_suffix), empty=True, world=True)
            cns_grp = maya.cmds.group(
                name=ctrl_name.replace(self._ctrl_suffix, self._auto_suffix), empty=True, world=True)
            ...
            jiggle = 1.0
            if i == 0 or i == (self._num_controls - 1):
                jiggle = 0.0
        ...
        # if self._constrain_mid:
        if not self._constrain_mid:
            for i in range(1, self._num_controls - 1):
                # get point constraint weight and create point constraint for intermediate controls
                pct = 1.0 * i / (self._num_controls - 1.0)
                maya.cmds.pointConstraint(self._controls[0].control, self._constraint_groups[i], weight=(1.0 - pct))
                maya.cmds.pointConstraint(
                    self._controls[self._num_controls - 1].control, self._constraint_groups[i], weight=pct)
                ...
                grp_aim_fwd = maya.cmds.group(
                    name='{}_aimFwd_{}_{}'.format(base_name,i, self._grp_suffix), empty=True, world=True)
                grp_aim_bck = maya.cmds.group(
                    name='{}_aimBack_{}_{}'.format(base_name, str(i), self._grp_suffix), empty=True, world=True)
                for grp in [grp_aim_fwd, grp_aim_bck]:
                    transform_utils.match_translation(grp, self._controls[i].control)
                maya.cmds.sets([self._root_groups[i], grp_aim_fwd, grp_aim_bck], include=set_rig)
                ...
                a_constraint = maya.cmds.aimConstraint(
                    self._controls[self._num_controls - 1].control, grp_aim_fwd, weight=1, aimVector=(0, 1, 0),
                    upVector=(1, 0, 0), worldUpVector=(1, 0, 0), worldUpType="objectrotation",
                    worldUpObject=self._controls[self._num_controls - 1].control)[0]
                ...
                maya.cmds.sets([a_constraint, b_constraint], include=set_rig)
                ...
                if i == 1:
                    ...
                    maya.cmds.sets(blend, include=set_rig)
                ...
                p_cons_fwd2 = maya.cmds.pointConstraint(
                    self._controls[self._num_controls - 1].control, grp_aim_fwd, weight=pct)[0]
                ...
                p_cons_back2 = maya.cmds.pointConstraint(
                    self._controls[self._num_controls - 1].control, grp_aim_bck, weight=pct)[0]
                maya.cmds.sets([p_cons_fwd1, p_cons_fwd2, p_cons_back1, p_cons_back2], include=set_rig)
                ...
                maya.cmds.sets([o_cons_bck, o_cons_fwd], include=set_rig)

                # create root grops for each one of the aim groups (so its xfrorms are zeroed out)
                grp_aim_fwd_root = maya.cmds.group(
                    name='{}_{}_grpAimFwd_{}'.format(base_name, self._muscle_spline_name, self._root_suffix),
                    empty=True, world=True)
                grp_aim_bck_root = maya.cmds.group(
                    name='{}_{}_grpAimBck_{}'.format(base_name, self._muscle_spline_name, self._root_suffix),
                    empty=True, world=True)
                ...
                maya.cmds.sets([grp_aim_fwd_root, grp_aim_bck_root], include=set_rig)
        ...
        for i in range(self._num_drivens):
            ...
            maya.cmds.sets(self._drivens[i], include=set_rig)
